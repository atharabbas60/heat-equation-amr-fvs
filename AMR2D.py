...
# 3. Sourse term
# ============================================================
def exact_solution(x, t): # Here exact solution u is actually a prescribed function used for initial and Dirichlet boundary data.
...
# Specific choice of source term for AMR
def forcing(x, t):
    x0 = x[0]
    x1 = x[1]
    r = np.linalg.norm(np.array([x0, x1]) - np.array([0.5, 0.5]))
    r = max(r, 1.0e-12)
    return (x0 * (1.0 - x0) * x1 * (1.0 - x1) * (np.exp(-t)+ t**2 / np.sqrt(r)))
# ============================================================
# 4. Geometry
# ============================================================
...
# ============================================================
def cell_residual_norm_sq(mesh, K, I_Mu_new, I_Mu_old,dt, time_value):
    verts = mesh.points[mesh.cells[K]]
    # Laplacian of the reconstructed solution at t^{n+1}
    lap_poly = laplacian_polynomial(I_Mu_new[K])
    # Time derivative of the Morley reconstruction
    poly_new = I_Mu_new[K]
    poly_old = I_Mu_old[K]
    def residual_function(x):
        time_term = (poly_new.evaluate_vec(x)- poly_old.evaluate_vec(x)) / dt
...
def tangential_jump_norm_sq_on_edge(mesh, K, E, I_Mu):
    ...
    jump_values = np.array([jump_function(p) for p in points])
    return np.dot(weights, jump_values ** 2)
def local_estimator_eta_K_sq(mesh,K,I_Mu_new,I_Mu_old,dt,time_value):
    verts = mesh.points[mesh.cells[K]]
    hK = rectangle_diameter(verts)
    residual_sq = cell_residual_norm_sq(mesh,K,I_Mu_new,I_Mu_old,dt,time_value)
    normal_part = 0.0
    tangential_part = 0.0
    for E in range(4):
        edge0, edge1 = edge_points_of_cell(mesh, K, E)
        ...
        if mesh.neighbors[K, E] != -1:
            Jn_sq = normal_jump_norm_sq_on_edge(mesh,K,E,I_Mu_new)
            normal_part += ((1.0 / hEK) * Jn_sq)
        Jt_sq = tangential_jump_norm_sq_on_edge(mesh,K,E,I_Mu_new)
        tangential_part += ((1.0 / hEK) * Jt_sq)
    return hK ** 2 * (residual_sq + normal_part + tangential_part)
def global_estimator_eta(mesh,I_Mu_new,I_Mu_old,dt,time_value):
    eta_sq = 0.0
    eta_local = np.zeros(mesh.nsimplices)
    for K in range(mesh.nsimplices):
        eta_local[K] = local_estimator_eta_K_sq(mesh,K,I_Mu_new,I_Mu_old,dt,time_value)
        eta_sq += eta_local[K]
    return np.sqrt(eta_sq), eta_local
# ============================================================
# MAXIMUM STRATEGY FOR 2D REFINEMENT
# ============================================================
def mark_xy_lines(mesh, eta_local, theta=0.2):
    ...
    for S in range(new_mesh.nsimplices):
        ...
        if center[0] < xc:
            ...
            if center[1] < yc:
                ...
                new_u[S] = uNE
    return new_u
def task(n=4,Tfinal=2.0,dt=0.2,tolerance=0.009,theta=0.2):
    print("=" * 90)
    ...
    while current_time < Tfinal:
        ...
        # OLD MORLEY RECONSTRUCTION
        I_Mu_old = reconstruct(mesh,u_current,current_time)
        # SOLVE ONE BACKWARD EULER TIME STEP
        u_next = solve_one_step(mesh,u_current,current_time,dt)
        t_new = current_time + dt
        # NEW MORLEY RECONSTRUCTION
        I_Mu_new = reconstruct(mesh,u_next,t_new)
        # ESTIMATOR
        eta_global, eta_local = global_estimator_eta(mesh,I_Mu_new,I_Mu_old,dt,t_new)
        print(
            f"eta = {eta_global:.10e}")
        # ------------------------------------------------
        # ACCEPT TIME STEP
        # ------------------------------------------------
        if eta_global < tolerance:
            ...
            current_time = t_new
        # ------------------------------------------------
        # REFINE AND REPEAT SAME TIME STEP
        # ------------------------------------------------
        else:
            print("REFINE MESH")
            ...
            refined_mesh = structured_meshes(nx=mesh.nx,ny=mesh.ny,
                xs=np.unique(mesh.points[:, 0]),
                ys=np.unique(mesh.points[:, 1]))
            # Refine complete vertical columns
            refined_mesh.refine_vertical_columns(x_targets)
            # Refine complete horizontal rows
            refined_mesh.refine_horizontal_rows(y_targets)
            mesh = refined_mesh
            # Transfer old solution to new mesh
            u_current = transfer_solution(old_mesh,old_u,mesh)
            print("nx =",mesh.nx)
            print("ny =",mesh.ny)
            # current_time is NOT changed, the same time step is solved again.
# ============================================================
# RUN TASK 
# ============================================================
if __name__ == "__main__":task(n=4,Tfinal=2.0,dt=0.2,tolerance=0.009,theta=0.2)